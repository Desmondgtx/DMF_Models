# -*- coding: utf-8 -*-
"""
Created on Mon Sep  1 22:33:52 2025
...
def run_simulation_with_G(G_value):
    ...
    return correlation, euclidean

# Búsqueda del óptimo
best_G = None
best_correlation = -1

for G in np.arange(0.8, 2.0, 0.1):
    print(f"Probando G = {G:.2f}")
    corr, eucl = run_simulation_with_G(G)
    
    if corr > best_correlation:
        best_correlation = corr
        best_G = G
        print(f"  Nuevo mejor: correlación = {corr:.3f}")

print(f"\nÓptimo: G = {best_G}, correlación = {best_correlation:.3f}")

# Ejecutar simulación final con G óptimo
DMF.G = best_G
